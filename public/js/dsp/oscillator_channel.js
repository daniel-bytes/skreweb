...
console.log(JSON.stringify(params))
    ...
    var lpffreq = (this.calcFrequency(params.lpf) * 1.25) + 200;
    ...
    this.hpf_param = new ParameterNode(this.audiolet, hpffreq);
    this.lpf_param = new ParameterNode(this.audiolet, lpffreq);
    ...
    this.delay_node = new FeedbackDelay(this.audiolet, 1.0, params.delay, params.feedback, 1);
    ...
    this.gain_node.connect(this.amp_mod_crossfade);
...
OscillatorChannel.prototype.setParameter = function(name, p) {    
    if (name === "osc_freq" || name === "freq_mod" || name === "hpf") {
        p = this.calcFrequency(p);
    }
    else if (name === "lpf") {
        p = (this.calcFrequency(p) * 1.25) + 200;
    }
    
    console.log("setting parameter %s to %s", name, p);
